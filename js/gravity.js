...
var AtomizedContent = function(){
	...
	function init(){
		...
		avoidMouse.setRadius( 70 );
		avoidMouse.strength = -2000;
		...
		point1Before = viewWidth / 2;
		point2Before = viewHeight / 2;
	...
	function draw(){
		...
		if (tenaciousUser == false) {

			context.fillStyle = 'rgba(249,249,249,0.8)';
			...
			for ( var i = 0, n = physics.particles.length; i < n; i++ ) {
				...
				if (mouseIsDown) {
					context.arc( particle.pos.x, particle.pos.y, particle.radius/1.4, 0, Math.PI * 2 );
					pullToCenter.strength = -220;
				}
				else {
	...
	function drawFavicon() {
	    ...
	    link.href = canvas2.toDataURL("image/x-icon");
	...
	function drawRandomShape(){
		...
		context.beginPath();
		context.moveTo(point1Before, point2Before);

		var point1 = point1Before + randomRange(-4,4);
		var point2 = point2Before + randomRange(-4,4);
	...
	function mouseMove(event){
		...
		avoidMouse.target.x = mouseX;
   		...
   		var deltaMouseX = Math.abs(mouseXBefore - mouseX); 
   		var deltaMouseY = Math.abs(mouseYBefore - mouseY); 

   		var deltaMouse = deltaMouseX + deltaMouseY;

   		mouseMovement = deltaMouse + mouseMovementBefore;

   		mouseValue = map_range(mouseMovement,0,10000,0,100);
		...
		$(function() {
   		...
   		if (mouseMovement >= 10000) {
   			mouseMovement = 0;
   			
   			tenaciousUser = true;
   			//alert("hej");
   			$("#secretBox").css({ "color": "#e5007d" });
   			$("#saveButton").css({ "background": "#e5007d" });
   			$("#saveButton").css({ "color": "#f9f9f9" });
   			$("#secretBox").hide();
   			$("#secretBox").fadeIn("slow");

   			console.log(document.getElementsByTagName("canvas"));
   			$( "#saveButton" ).click(function() {
   			var canvas = document.getElementsByTagName("canvas");
			var image = canvas[0].toDataURL("image/png").replace("image/png", "image/octet-stream"); //Convert image to 'octet-stream' (Just a download, really)
   		...
   		mouseXBefore = mouseX;
   		mouseYBefore = mouseY;
   		mouseMovementBefore = mouseMovement;
